 ...
 * using Spring. The endpoints you will need can be
...
public class SocialMediaController {
    ...
    public ResponseEntity<String> postMessage(@RequestBody String jsonMessage) throws JsonProcessingException {
        ...
        Message messageAttempt = messageService.postMessage(registered.getPostedBy(), registered.getMessageText(),
                registered.getTimePostedEpoch());
    ...
    public ResponseEntity<String> updateMessageById(@PathVariable String messageId, @RequestBody String json)
            throws JsonProcessingException {
        ObjectMapper mapper = new ObjectMapper();
        // help read in generic Json string without having to create a speciality class
        JsonNode jn = mapper.readTree(json);
        boolean messageChanged = messageService.updateMessage(Integer.parseInt(messageId),
                jn.get("messageText").asText());
        if (messageChanged) {
            return ResponseEntity.status(200).body("1");
        } else {
            return ResponseEntity.status(400).body(null);
